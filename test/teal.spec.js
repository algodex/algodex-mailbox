...
const config = {
  ...
  unfundedReceiverAccount: testHelper.getRandomAccount(),
...
console.debug('unfunded receiver account: ' + config.unfundedReceiverAccount.addr)
...
const negativeFundEscrowWithoutNoteTests = [
  {txnNum: 0, field: 'from', txnKeyForVal: 'from', txnNumForVal: 1}, //set to from escrow
  {txnNum: 0, field: 'amount', val: 100000},
  {txnNum: 0, field: 'to', val: algosdk.decodeAddress(config.maliciousAccount.addr) },
  {txnNum: 0, field: 'reKeyTo', val: algosdk.decodeAddress(config.maliciousAccount.addr) },
...
describe('Test Mailbox Funding And Withdrawal', () => {
  ...
  test('Setup', async () => {
    ...
    let receiverAccountInfo = null
    do {
      receiverAccountInfo = await algodex.getAccountInfo(receiverAccount.addr)
      console.log('Sleeping 1 second while waiting for indexer')
      await sleep(1000)
    } while (!receiverAccountInfo?.amount)
  ...
  negativeFundEscrowWithoutNoteTests.map( (negTestTxnConfig) => {
    const {
      senderAccount,
      unfundedReceiverAccount,
      //openAccount,
      //maliciousAccount,
      client,
      assetId,
    ...
    test (testName, async () => {
      ...
      const outerTxns = await generateTxns.getFundEscrowTxns(
        ...
        100000,
        senderAccount,
        unfundedReceiverAccount.addr
      )
      expect(outerTxns.length).toBe(3)
  ...
  test('Return To Sender', async () => {
    ...
    let txns = await generateTxns.getReturnToSenderTxns(client, assetId, 
      receiverAccount2.addr, senderAccount)
    // const firstTxn = txns[0].unsignedTxn
    let signedTxns = await testHelper.groupAndSignTransactions(txns)
    ...
    txns = await generateTxns.getReturnToSenderTxns(client, assetId, 
      unfundedReceiverAccount.addr, senderAccount)
    signedTxns = await testHelper.groupAndSignTransactions(txns)
