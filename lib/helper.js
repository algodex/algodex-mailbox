...
const Helper = {
  ...
  signWithWalletConnect: async function (txnsForSigning, allTxns, connector) {
    ...
    const txns = allTxns.filter((txn) => txn.lsig)
    const innerTxnsForSigning = txns.map((txn) => {
      ...
      return {
        txn: encodedTxn,
        message: 'Send transaction on Algodex mailbox',
      }
    })

    // console.log('innerTxnsForSigning re', innerTxnsForSigning)
    const request = formatJsonRpcRequest('algo_signTxn', [innerTxnsForSigning])

    const signedTxnsFromUser = await connector.sendCustomRequest(request)
    // const decodedResult = signedTxnsFromUser.map((element) => {
