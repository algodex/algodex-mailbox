...
const Helper = {
  ...
  checkOptIn: async function (addr, assetId) {
    ...
    let nextToken = ''
    do {
      const url = indexerServer + indexerPort + `/v2/accounts/${addr}/assets` + nextToken
      const response = await axios.get(url,
        { headers: { 'X-Algo-API-Token': indexerToken } }
      )
      assets = [...assets, ...response.data.assets]
      nextToken = response.data['next-token'] ? `?next=${response.data['next-token']}` : ''
  ...
  getAlgoNamesOrAddress: async function (address, type) {
    ...
    const hostname = environment === 'production' ? 
      'https://api.nf.domains' : 'https://api.testnet.nf.domains'

    const url = type === 'getNames' ? hostname + '/nfd/' + address : 
