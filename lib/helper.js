...
const Helper = {
  ...
  getAlgodex: function () {
    if (!this.envObjs) {
      ...
      const purestakeIndexerServer = this.getEnvironment() === 'mainnet'
        ? 'https://mainnet-algorand.api.purestake.io/idx2'
        : 'https://testnet-algorand.api.purestake.io/idx2'

      const purestakeClientServer = this.getEnvironment() === 'mainnet'
  ...
  searchAlgoAssets: async function (keywords) {
    const { indexerToken } = this.getAlgodex()
    const url = 'http://localhost:3000/algod'
    try {
