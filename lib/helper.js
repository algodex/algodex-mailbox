...
const Helper = {
  ...
  packageUnsignedTransactions: function (
    ...
    for (let i = 0; i < outerTxns.length; i++) {
      ...
      events.emit('getting-balance', {
        status: 'Looking up wallet balances & signing logical signatures...',
        progress: i,
  ...
  signWithMyAlgo: async function (txnsForSigning, allTxns) {
    events.emit('sign-with-my-algo', {
      status: 'Waiting for user signature via the MyAlgo GUI..',
      progress: 0,
      total: txnsForSigning.length / 2,
    })
  ...
  sendAllTransactions: async function (allTransList) {
    events.emit('send-all-transactions', {
      status: 'Sending the transactions to the recipients on the blockchain...',
      progress: 1,
      total: allTransList.length,
    })
