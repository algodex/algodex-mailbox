...
const Helper = {
  ...
  /**
   * Use constructed transactions
   * @param t
   * @return {*}
   */
  transactionFactory: function (t) {
    const {
      from,
      to,
      closeRemainderTo,
    ...
    const method = {
      axfer: algosdk.makeAssetTransferTxn
    }
    //
    const inputs = {
      axfer: [
        from, to, closeRemainderTo, revocationTarget, fee, amount, firstRound,
        lastRound, note, genesisHash, genesisID, assetIndex, rekeyTo
      ]
    }
    return method[t.type](...inputs[t.type])
  },
  /**
  ...
  signWithSDK: async function (txnsForSigning, allTxns, totalLength) {
    ...
    const signedTxnsFromUser = txnsForSigning.map((txn)=>{
      return algosdk.signTransaction(
        this.transactionFactory(txn.unsignedTxn),
