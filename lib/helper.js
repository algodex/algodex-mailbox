...
const Helper = {
  ...
  signWithMyAlgo: async function(txnsForSigning, allTxns) {
    ...
    events.emit('sign-with-my-algo', {status: 'signing'})
    ...
    events.emit('sign-with-my-algo', {status: 'signed'})
    console.debug({ signedTxnsFromUser })
    //const outerGroupTo
    // events.emit('sending-assets-event', {status: 'signing2'})
    ...
    for (let i = 0; i < allTxns.length; i++) {
      ...
      if (trans.lsig) {
        ...
        events.emit('sign-with-my-algo', {
          status: 'Signing with Trasnaction', progress: i, total:allTxns.length
        })
  ...
  sendAllTransactions: async function (allTransList) {
    events.emit('sending-assets-event', {status: 'sending'})
      ...
      if (i === allTransList.length - 1) {
        ...
        if (signedTxns.length > 0) {
          try {
            ...
            events.emit('sending-assets-event', {
              status: 'Sending assets', progress: i, total:allTransList.length
            })
          } catch (e) {
            console.debug(e)
            return e.response
  ...
  getShareableRedeemLink: function (walletAddr, assetId, receiverAddress=null) {
    let webURL = ''
  ...
  getFormattedAssetAmount : async function (assetBalance, assetId, showName = false) {
  ...
  formatTransactionsWithMetadata: async function (
    ...
    let acceptedTxnTypes = ['initialFundEscrow','addToEscrow', 'withdraw', 'sendDirect']
    ...
    if (txnType === 'fund') {
      const formattedAmount = await this.getFormattedAssetAmount(assetAmount, assetId, true)
      const shareLink = this.getShareableRedeemLink(senderAddr, assetId, receiverAddress)
      retTxns[retTxns.length - 1].unsignedTxn.note =
        enc.encode(`You have received ${formattedAmount.balance} `
