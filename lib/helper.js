...
const algodexImport = require('@algodex/algodex-sdk-patch')
const testHelper = require('@algodex/algodex-sdk-patch/test_helper.js')
const algodexApiImport = require('@algodex/algodex-sdk-patch/algodex_api.js')
...
const Helper = {
  ...
  getAlgodex: function () {
    if (!this.envObjs) {
      ...
      const purestakeIndexerToken =
        process.env.NEXT_PUBLIC_CUSTOM_INDEXER_TOKEN
  ...
  checkOptIn: async function (addr, assetId) {
    ...
    do {
      const url =
        indexerServer + indexerPort + `/v2/accounts/${addr}/assets` + nextToken
    ...
    const hasAsset = !!assets.find((asset) => asset['asset-id'] === assetId)
    return hasAsset
  },
    ...
    for (let i = 0; i < outerTxns.length; i++) {
      ...
      if (!outerTxns[i].lsig) {
        ...
        events.emit('sign-with-my-algo', {
          status:
            'Sending transactions to connect bridge for signing... (Step 2/4)',
          progress: groupCount,
          total: totalLength,
        })
      }
  ...
  signWithWalletConnect: async function (txnsForSigning, allTxns, connector) {
    events.emit('sign-with-my-algo', {
      status:
        'Waiting for user signature, check your wallet to confirm.. (Step 3/4)',
      progress: 0,
    })

    const encodedTxns = allTxns.map((txn) => {
      const encodedTxn = Buffer.from(
        algosdk.encodeUnsignedTransaction(txn.unsignedTxn)
      ).toString('base64')
      // eslint-disable-next-line max-len
      if (txn.lsig) return { txn: encodedTxn, signers: [] } // naive solution assuming target address is first item in accountds array
      ...
      return element
        ...
        : {
          ...algosdk.signLogicSigTransactionObject(
            allTxns[idx].unsignedTxn,
            allTxns[idx].lsig
          ),
        }
    })

    const outerTxnsWithResults = allTxns.map((txn, i) => {
      return { ...txn, signedTxn: resultsFormatted[i].blob }
    })

    events.emit('sign-with-my-algo', { status: 'Complete' })
    // return allTxns
    return outerTxnsWithResults
  },
  ...
  convertToMyAlgoTxn: function (txn) {
    ...
    if (txn.lsig || txn.senderAcct.type !== 'myAlgo') {
      return txn
    }
  ...
  getAlgoNamesOrAddress: async function (address, type) {
    ...
    const url =
      type === 'getNames'
        ? hostname + '/nfd/v2/address?address=' + address
        : hostname + '/nfd/' + address

    try {
      const fetchval = await axios.get(url)
      if (type === 'getNames') {
        return fetchval.data[address][0].name
      } else {
        return fetchval.data.depositAccount || 'UNKNOWN'
      }
    } catch (error) {
      if (type === 'getNames') {
        return address
