...
const Helper = {
    ...
    getAssetUnitName: async function(assetId) { 
        ...
        const ALGOD_PORT = '';
        const response = await axios.get(ALGOD_INDEXER_SERVER + ALGOD_PORT + 
            "/v2/assets/"+assetId, {headers: {'X-Algo-API-Token': ALGOD_INDEXER_TOKEN}});
    ...
    getFormattedAssetBalanceFromAmountAndDecimals: function(amount, assetId, assetDecimals) {
    ...
    getFormattedAssetBalance : async function(addr, assetId, showName = false) {
        ...
        const formattedBalance = this.getFormattedAssetBalanceFromAmountAndDecimals(assetBalance, assetId, assetDecimals);
        const unitName = showName ? ' ' + await this.getAssetUnitName(assetId) : '';
        return formattedBalance + unitName;
    },
