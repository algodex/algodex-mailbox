...
const GenerateTransactions = {
  ...
  getSimpleSendAlgosTransaction : async function (client, algoAmount, fromAccount, toAddress) {
    const params = await this.getParams(client)
    const outerTxns = []
    let paymentTxn = algosdk.makePaymentTxnWithSuggestedParams(fromAccount.addr,
      toAddress, algoAmount, undefined, undefined, params, undefined)
  ...
  getFundEscrowTxns : async function (client, assetId, assetAmount, fromAccount, toAddress) {
    ...
    try {
      const receiverAccountInfo = await algodex.getAccountInfo(toAddress)
    ...
    const sendNote = receiverBalance > 0
    ...
    let assetFundTxn = null
    
    if (sendNote) {
      // Pay double fee since it needs to pay for the note
      assetFundTxn = algosdk.makeAssetTransferTxnWithSuggestedParams(fromAccount.addr,
        lsig.address(), undefined, undefined, assetAmount, note, assetId, 
        doubleFeeParams, undefined)
    } else {
      assetFundTxn = algosdk.makeAssetTransferTxnWithSuggestedParams(fromAccount.addr,
        lsig.address(), undefined, undefined, assetAmount, note, assetId, 
        params, undefined)
    ...
    const sendType = outerTxns.length > 2 ? 'initialFundEscrow' : 'addToEscrow'
    return await helper.formatTransactionsWithMetadata(
      sendNote,
      outerTxns,
      fromAccount.addr,
