...
const SendAssets = {
  getMailboxSendTransactions: async function (
    ...
    if (isOptedIn) {
      return await generateTransactions.getSimpleSendAssetTxn(
        algodClient,
        assetId,
        amount,
        fromAccount,
  ...
  send: async function (assetId, fromAddress, csvInput) {
    try {
      ...
      events.emit('sending-assets-event', { status: 'loading' })
      ...
      for (let k = 0; k < toData.length; k++) {
        ...
        const outerTxns = await this.getMailboxSendTransactions(
          algodClient,
          assetId,
          amount,
          fromAccount,
      ...
      // Add operator fee transaction for direct transfers of asset
      ...
      events.emit('sending-assets-event', { status: 'signing1' })
      const signedAllTxns = await helper.signWithMyAlgo(
        txnsForSigning,
        allTxns
      )
      ...
      events.emit('sending-assets-event', { status: 'complete' })
      return {
        error: false,
        confirmedTransactions,
      }
    } catch (error) {
      events.emit('sending-assets-event', { status: 'stop' })
      return error
    }
  },
}
