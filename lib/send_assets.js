  ...
  send: async function (assetId, fromAddress, csvInput) {
    try {
      ...
      if (simpleAssetTransferCount > 0) {
        ...
        const outerSendTxn = await generateTransactions.getSimpleSendAlgosTransaction(algodClient, 
          operatorFee, fromAccount, feeReceiverAddress)
        helper.packageUnsignedTransactions(allTxns, txnsForSigning, toData.length, outerSendTxn)
      }
