...
const TransactionHistory = {
  getCSVRows: async function (assetId, senderAddress, sendType) {
    try {
      ...
      let loopCount = 0
      do {
        const url = `${indexerServer}${indexerPort}/v2/accounts/${senderAddress}` + 
  ...
  getTransactionHistory: async function (assetId, senderAddress) {
    ...
    const csvHeader = 'Time,FromAddress,ToAddress,Amount,Escrow,SendType,AssetId,TransactionId\n'
    let csvRows = ''
    for (let i = 0; i < sendTypes.length; i++) {
      csvRows += await this.getCSVRows(assetId, senderAddress, sendTypes[i])
    }
    ...
    return csvRowsArr.reduce( (previous, row) => previous + row + '\n', csvHeader)
  }
}
