...
const TransactionHistory = {
  getCSVRows: async function (assetId, senderAddress, sendType) {
    try {
      const { purestakeIndexerServer, indexerPort, purestakeIndexerToken } = helper.getAlgodex()
      // Use Purestake since Algoexplorer disabled this type of lookup
      const indexerServer = purestakeIndexerServer
      ...
      do {
        const url =
          `${indexerServer}${indexerPort}/v2/accounts/${senderAddress}` +
