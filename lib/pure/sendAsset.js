...
/**
 * Send asset from a Wallet
 *
 * Takes a CSV input to map to available destinations
 *
 * @param {Algodv2} client Algorand SDK
 * @param {Asset} asset Algorand Asset
 * @param {Wallet} wallet Send from Wallet
 * @param csvInput
 * @returns {Promise<{error: boolean, confirmedTransactions: unknown[]}|any>}
 */
async function sendAsset(client, asset, wallet, csvInput) {
  ...
  try {
    //const formattedBalance = await helper.getFormattedAssetBalance(fromAddress, assetId, true);
    ...
    let lastChar = csvInput[csvInput.length - 1]
    if (lastChar !== '\n') {
      csvInput += '\n'
    }
      ...
      if (lastGroupNum !== allTransList[i]['groupNum']) {
        ...
        if (signedTxns.length > 0) {
          ...
          try {
            ...
            let txn = await client.sendRawTransaction(signedTxns).do()
    ...
    let confirmedTransactions = await algodex_api.allSettled(
